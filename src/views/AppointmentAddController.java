...
/**
 * AppointmentAddController Class
 *
 * @author David Roeske
 */
public class AppointmentAddController implements Initializable {
    ...
    private final ObservableList<String> listOfTimes = FXCollections.observableArrayList("9:00 AM","10:00 AM","11:00 AM","12:00 PM","1:00 PM","2:00 PM","3:00 PM","4:00 PM");   //Active times for appointments
    ...
    //***APPOINTMENT ADD SCREEN ELEMENTS***
    //TEXTFIELDS
    ...
    //***METHOD TO GET CUSTOMER NAME - CALLED IN MAIN CONTROLLER***
    public void getNameofCustomer(String name) {
        customerName.setText(name); // For appointment fields
    }
    ...
    public boolean addAppointmentAction(int contactid) {
        ...
        if(AppointmentUtilization.overlappingAppointment(-1, appointmentLocation.getText(), date.toString(), listOfTimes.get(time))) {
            return false;
        }
        if(!isValidEntries(contact, title, description, time, date)) {
            return false;
        }
        if(AppointmentUtilization.addAppointment(contactid, listOfTitles.get(title), listOfDescriptions.get(description), listOfContacts.get(contact), appointmentLocation.getText(), date.toString(), listOfTimes.get(time), listOfDurations.get(duration))) {
            return true;
        } else {
            return false;
        }
    ...
    @Override public void initialize(URL url, ResourceBundle rb) {
        ...
        @Override public void updateItem(LocalDate thedate, boolean isblank) {
                ...
                if(thedate.getDayOfWeek() == DayOfWeek.SUNDAY || thedate.getDayOfWeek() == DayOfWeek.SATURDAY || thedate.isBefore(LocalDate.now())) {
                    setStyle("-fx-background-color: #6e1515;"); // Cannot select these 
                }
            }
        });
...
} // End AppointmentAddController Class
